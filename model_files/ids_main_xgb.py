...
def load_cicids2017_data(file_path):
    try:
        ...
        np.save('saved_models/features_used_xgb.npy', X.columns.to_numpy())
        with open('saved_models/scaler_xgb.pkl', 'wb') as f:
...
np.save('saved_models/X_test_xgb.npy', X_test)
np.save('saved_models/y_test_xgb.npy', y_test)

# Train XGBoost
xgb_clf = xgb.XGBClassifier(n_estimators=100, eval_metric='logloss', random_state=42, n_jobs=-1)
...
# Save the model
joblib.dump(xgb_clf, 'xgb_intrusion_detection_model.joblib')
print("Model saved as 'xgb_intrusion_detection_model.joblib'") 